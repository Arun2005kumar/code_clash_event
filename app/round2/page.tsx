...
export default function Round2Page() {
  ...
  const loadState = useCallback(async () => {
    ...
    const qNum = settings?.current_round2_question || 1;
    ...
    if (!q) {
      q = ROUND2_FALLBACK_QUESTIONS[qNum] || ROUND2_FALLBACK_QUESTIONS[1];
    }
  ...
  const handlePlaceBid = async (increment: number) => {
    ...
    const targetQId = (currentQuestion.id && currentQuestion.id.includes('-'))
      ? currentQuestion.id
      : `00000002-0000-0000-0000-00000000000${currentQuestion.question_number || 1}`;
  ...
  return (
    <AntiCheatGuard teamId={session.teamId} teamName={session.teamName} roundName="Round 2">
