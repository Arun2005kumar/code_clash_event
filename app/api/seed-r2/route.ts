...
export async function GET() {
  ...
  const { data, error } = await supabase
    ...
    .upsert(round2Questions, { onConflict: 'question_number' });

  if (error) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }

  // Also ensure settings current question exists
  await supabase
    .from('competition_settings')
    .upsert({ id: '00000000-0000-0000-0000-000000000001', current_round2_question: 1, round2_active: true });

  return NextResponse.json({ success: true, count: round2Questions.length, message: 'All 6 Hard Trick Questions seeded successfully for Round 2!' });
}
